...
function pickTimeUnit(series: SeriesSpec[]): TimeUnit {
	const times = [...new Set(series.flatMap((s) => s.points.map((p) => Number(p.x))))].sort((a, b) => a - b);
...
export function renderChart(canvas: HTMLCanvasElement, spec: ChartSpec): void {
	...
	const numberFmt = (v: number) => v.toLocaleString(undefined, { maximumFractionDigits: 2 });
	...
	const timeLabel = (value: unknown) =>
		spec.rawLabels.get(Number(value)) ?? formatDate(new Date(Number(value)), TICK_PATTERNS[timeUnit]);
			...
			scales: {
				x: isTime
					? {
							...
							ticks: { source: 'data', color: tickColor, maxRotation: 0, autoSkip: true, autoSkipPadding: 16, callback: timeLabel },
						}
					: {
							type: 'category',
							grid: { color: gridColor },
