...
export function detectTimeAxisFor(
  rows: string[][],
  orientation: Orientation
): TimeAxisHit | null {
...
export function analyze(
  ...
  if (mode === "time") {
    format = mapping.formatOverride
      ? makeTokenFormat(mapping.formatOverride)
      : (detectAxisFormat(axisCells)?.format ?? null);
    if (format) {
    ...
    positions = axisCells.flatMap((c, i) =>
      c && c.trim() && numericAnywhere.has(i) ? [i] : []
    );
  }
