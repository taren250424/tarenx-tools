...
export const FORMATS: DateFormat[] = [
	{
		...
		parse(raw) {
			const m = raw.match(/(\d{4})\s*[.\-/년]?\s*(?:Q\s*([1-4])|([1-4])\s*\/\s*4(?!\d)|([1-4])\s*분기)/i);
			if (!m) return null;
			const y = Number(m[1]);
	...
	},
	{
		...
		label: 'YYYY-MM-DD',
		parse(raw) {
			const m = raw.match(/(\d{4})\s*[.\-/년]\s*(\d{1,2})\s*[.\-/월]\s*(\d{1,2})(?!\d)/);
			if (!m) return null;
			const y = Number(m[1]);
			const mo = Number(m[2]);
			const d = Number(m[3]);
...
export function parseAxisWith(cells: string[], format: DateFormat): AxisParse {
	...
	return { format, dates, hits: seq.length, score: seq.length * (0.5 + 0.5 * monoRatio) * coverage };
}
