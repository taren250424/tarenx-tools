...
import { analyze, detectTimeAxis, detectTimeAxisFor, guessOrientation, seriesLabel, toNumber } from './detect.ts';
import type { SeriesSpec } from './chart.ts';
import { chartToPngBlob, clearChart, defaultColor, renderChart } from './chart.ts';
...
function configureMapping(source: Source, forced?: Orientation, updateMode = false): void {
	const hit = forced ? detectTimeAxisFor(source.rows, forced) : detectTimeAxis(source.rows);
	const orientation = forced ?? (hit ? hit.orientation : guessOrientation(source.rows));
	source.mapping = {
		orientation,
	...
	for (const row of analysis.candidates.slice(0, AUTO_SELECT_LIMIT)) source.mapping.selected.add(row);
}

function addSource(name: string, buffer: ArrayBuffer | null, text: string): void {
	...
	const source: Source = {
		...
		rows,
		mapping: { orientation: 'horizontal', axisIndex: 0, selected: new Set(), colors: new Map(), formatOverride: null },
	};
...
function removeSource(id: number): void {
	...
	if (activeId === id) activeId = sources.length > 0 ? sources[sources.length - 1].id : -1;
	renderAll();
}
...
function renderAll(): void {
	...
	els.modeCategoryBtn().setAttribute('aria-pressed', String(mode === 'category'));
...
function renderActive(): void {
	...
	for (const row of source.mapping.selected) {
		const color = assignedColors.get(`${source.id}:${row}`) ?? source.mapping.colors.get(row);
		if (color) seriesColors.set(row, color);
	}
	renderTable(els.tableWrap(), source.rows, source.mapping, analysis, seriesColors, {
		setAxis(gridIndex) {
...
function renderReport(source: Source, analysis: Analysis): void {
	...
	const inRows = segButton('Series in rows', source.mapping.orientation === 'horizontal', () => {
		configureMapping(source, 'horizontal');
		renderAll();
	});
	inRows.title = 'Each row is one series; X-axis labels (dates/categories) run along a row';
	const inColumns = segButton('Series in columns', source.mapping.orientation === 'vertical', () => {
		configureMapping(source, 'vertical');
		renderAll();
	});
	inColumns.title = 'Each column is one series; X-axis labels (dates/categories) run down a column';
	...
	if (source.buffer) {
		...
		select.addEventListener('change', () => changeEncoding(source, select.value));
	...
	if (mode === 'time') {
		if (!formatLabel) {
			status.textContent = 'No time format detected on the X-axis line — pick another line, enter a format, or switch to Bars · Category.';
			status.classList.add('report-warn');
		} else {
			const skippedNote =
				skipped.length > 0 ? ` · skipped: ${skipped.slice(0, 4).join(', ')}${skipped.length > 4 ? '…' : ''}` : '';
	...
	if (mode === 'time') {
		...
		input.title = 'Tokens: YYYY, MM, DD, Q — numbers in each label are matched to tokens in order';
		const apply = () => {
...
function renderChartFromSources(): void {
	...
	for (const source of sources) {
		...
		const fallback = source.mapping.orientation === 'horizontal' ? 'Row' : 'Column';
		for (const row of [...source.mapping.selected].sort((a, b) => a - b)) {
			if (!analysis.grid[row]) continue;
			const points = analysis.positions.map((p) => ({
				x: mode === 'time' ? analysis.dates![p]!.getTime() : analysis.axisCells[p],
				y: toNumber(analysis.grid[row][p]),
			}));
			if (points.some((pt) => pt.y !== null)) {
				const color = source.mapping.colors.get(row) ?? defaultColor(series.length);
				nextAssigned.set(`${source.id}:${row}`, color);
				series.push({ label: prefix + seriesLabel(analysis, row, fallback), color, points });
			}
		}
	}
	...
	renderChart(els.canvas(), { mode, indexed, series, categories, rawLabels, yLabel });
}
	...
	if (dataPanel) {
		...
		handle.addEventListener('pointerdown', (e) => {
			...
			const onMove = (ev: PointerEvent) => {
				const height = Math.min(Math.max(160, startHeight + ev.clientY - startY), window.innerHeight * 0.8);
